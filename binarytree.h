...
struct BinaryTree {
 ...
 public:
  ...
  Node *remove(Node *r, const T &v) {
    ...
    // Если значение равно, то есть мы нашли нужный элемент
    assert(v == r->value);
  ...
  // Высота дерева - считаем напрямую
  int height(Node *n) {
    if (n == nullptr) return 0;  // Для пустого дерева 0
  ...
  struct CellDisplay {
    ...
    explicit CellDisplay(Node *n) : present(n) {
      ...
      // valStr = (n) ? to_string(n->value) + " " + to_string(n->height) + "/" + to_string(n->dis) : "";
    };
  };
  ...
  // row_formatter takes the vector of rows of CellDisplay structs
  // generated by getDisplayRows and formats it into a test representation
